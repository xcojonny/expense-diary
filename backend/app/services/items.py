...
async def get_or_create_item(
    session: AsyncSession, *, display_name: str, category_id: int | None
) -> Item | None:
    """Stammdatum zum normalisierten Namen holen oder anlegen.
    ...
    existing = (
        await session.execute(sa.select(Item).where(Item.normalized_name == normalized))
    ).scalar_one_or_none()
    ...
    item = Item(
        normalized_name=normalized, display_name=display_name.strip(), category_id=category_id
    )
    session.add(item)
    await session.flush()
    return item
...
async def apply_product_mapping(session: AsyncSession, line_item: LineItem) -> None:
    ...
    item = await get_or_create_item(
        session, display_name=line_item.name, category_id=line_item.category_id
    )
